...
class LogisticRegression:
    ...
    def train(self,max_iterations=1000):
        ...
        for label_index,unique_label in enumerate(self.unique_labels):
            # 修復：使用 flatten() 將 theta 轉為一維數組
            current_initial_theta = np.copy(self.theta[label_index].flatten())
            ...
            self.theta[label_index] = current_theta.flatten()  # 確保存儲時也是正確的形狀
    ...
    def gradient_descent(data,labels,current_initial_theta,max_iterations):
        ...
        if not result.success:
            raise ArithmeticError('Can not minimize cost function: ' + str(result.message))
