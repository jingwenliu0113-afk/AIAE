...
#: Report 13's cold-start record names a local checkpoint directory, so it is
#: per-run evidence and stays in the private research tree. The two tests that
#: read it are artifact-only; the sanitisation logic they exercise is also
#: driven directly, against a synthetic message, in the same file.
_COLDSTART_REPORT = (Path(__file__).resolve().parents[1]
                     / "data" / "reports" / "13_lora_coldstart.json")

ARTIFACT_ONLY = "artifact-only:"

needs_coldstart_report = pytest.mark.skipif(
    not _COLDSTART_REPORT.exists(),
    reason=f"{ARTIFACT_ONLY} report 13's cold-start record is not in this tree")
...
class TestColdStartLoadOrder:
    ...
    def _fakes(self, monkeypatch, calls):
        ...
        class FakePeftModel:
            @staticmethod
            def from_pretrained(model, path, revision=None, **kw):
...
def test_coldstart_report_refusal_message_uses_a_repository_relative_path():
    ...
    # The rewrite must be stated in the record, not left for a reader to
    # discover -- a message that reads like an exception will be quoted as one.
    note = mod._sanitization_note()
    assert note["refusal_message"]["verbatim"] is False
    assert note["fields"] == ["refusal_message"]
    assert note["refusal_message"]["original_retained"] is False


@needs_coldstart_report
def test_the_committed_coldstart_report_declares_its_sanitization():
    """Its message was cleaned after the run; the file has to say so."""
    import json

    root = Path(__file__).resolve().parents[1]
    report = root / "data" / "reports" / "13_lora_coldstart.json"
    stored = json.loads(report.read_text())

    note = stored["sanitization"]
    assert note["applied_after_the_run"] is True
    assert note["refusal_message"]["verbatim"] is False
    assert note["refusal_message"]["original_retained"] is False
    # And the thing being protected must actually be gone.
    assert "/Users/" not in report.read_text()


@needs_coldstart_report
def test_no_report_claims_a_de_identified_message_is_verbatim():
    """Rewriting at write time is still rewriting.

    The note here once said later runs "are verbatim" because their path is
    replaced at source rather than afterwards. That is a claim about *when*
    the edit happened, not about whether the stored string is the exception's
    literal text -- and it is the latter that a reader would take from the
    word. Both the stored report and the script that writes future ones must
    keep saying so.
    """
    import json
    ...
    root = Path(__file__).resolve().parents[1]
    stored = json.loads(
        (root / "data" / "reports" / "13_lora_coldstart.json").read_text())
    note = stored["sanitization"]["note"]
    assert "are verbatim" not in note
    assert "not verbatim either" in note

    import importlib.util

    spec = importlib.util.spec_from_file_location(
        "s13_verbatim", root / "scripts" / "13_lora_coldstart.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    future = mod._sanitization_note()
    assert future["refusal_message"]["verbatim"] is False
    assert "are verbatim" not in future["note"]
