...
class TestReportDoesNotOverclaim:
    ...
    def sources(self):
        return {"the script": SCRIPT.read_text(),
                "the report": (ROOT / "data" / "reports"
                               / "14_mps_speed.md").read_text()}

    def test_no_banned_phrasing_survives(self):
        for where, text in self.sources().items():
            for phrase, why in self.BANNED:
                assert phrase not in text, f"{where}: {phrase!r} -- {why}"

    def test_the_report_still_names_the_order_confound(self):
        text = self.sources()["the report"]
        assert "confounded with condition" in text
        assert "n=1" in text

    def test_the_report_states_that_optimizer_updates_run(self):
        assert "Optimizer updates do run" in self.sources()["the report"]

    def test_the_stored_run_is_still_labelled_four_decimal(self):
        """Its losses were rounded on the way to disk; that cannot change."""
        text = self.sources()["the report"]
        assert "stored precision of 4 decimal places" in text
...
class TestContractsAreIndependent:
    ...
    @pytest.mark.skipif(
        not (ROOT / "data" / "processed" / "instruct_inv_train.jsonl").exists(),
        reason="instruction data is not in git; replay checks its digest")
