...
→ CP-SAT 依手動庫存重新鋪磚，並在 formulation 內要求單一 stud 連通元件
→ 獨立重驗 exact cover／庫存／碰撞／邊界／接觸地面／連通
→ 第一個通過的結果輸出 LDraw 與預覽
```

每次嘗試分開記錄 `OPTIMAL`／`FEASIBLE`／`INFEASIBLE`／`UNKNOWN`、求解時間、
候選 placement 數、formulation 是否已強制連通、是否確實回傳鋪排，以及每一項
checker 結果。連通約束採單一 commodity flow，流只能通過兩個都被選中的相鄰層
placement；輸出後仍用獨立 checker 重驗。若 solver 的回傳內容遭替換成斷開鋪排，
仍會停在「有鋪排但未通過 checker」，不會發布；逾時也不會被寫成無解。
...
| F-oracle／F-pipeline | F-oracle 既有；F-pipeline 已採連通性感知 CP-SAT | F-pipeline 尚未正式評估；未改寫歷史 F-oracle |
| 碰撞、接地與連通檢查 | 完成；連通在求解內強制、輸出後再驗 | 接地與連通都不是物理支撐或穩定性 |
...
模型線維持 `final_H2`，不再訓練、調參、多 seed 或重選模型。Phase 3C 已於第
六十五輪執行完成（`gen09`），碰撞／連通拒絕層因此有了一次正式指標，而方向是
負的：Core Success@4 −5.0pp、95% 區間 [−9.4, −0.6]，區間不含 0。本文件與兩支
命令都不得解讀為它改善了任何 Success@K。
