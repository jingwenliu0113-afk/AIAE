...
> 本文件是 BrickAgain 專題的主要規劃文件，供專題本人、Codex、Claude 或其他協作者閱讀與接手。
>
...
- [ ] 決定第一版零件類別清單。
- [ ] 建立 `vision_class -> Rebrickable -> LDraw -> BrickNet` 對照表。
- [ ] 整理合成圖片。
- [ ] 拍攝真實單顆照片。
- [ ] 自己建立約 200～500 張真實照片的 Fine-tune／Test 集；實際數量依拍攝能力記錄，不得只報合成資料結果。
- [ ] 依實體零件或拍攝 Session 切分資料。
- [ ] 建立影像增強。
- [ ] 訓練 Transfer Learning Baseline。
- [ ] 實作傳統 CV Baseline，與 Transfer Learning 比較。
- [ ] 建立 Confusion Matrix。
- [ ] 分析易混淆零件。
- [ ] 測試 Top-3 候選供人工修正。
...
- [ ] 建立低信心人工選擇。
- [ ] 評估透明、黑色、白色及相近顏色。
- [ ] 實作結構槽位到現有 `(形狀, 顏色)` 庫存的配色器。
- [ ] 當某形狀的總顏色庫存不足時，回報無法配色，不得虛構顏色。
...
- [ ] 建立作品文字文件。
- [ ] 建立類別、顏色、零件數 Metadata。
- [ ] 選擇多語 Embedding 模型。
- [ ] 建立 FAISS、Chroma 或其他向量索引。
- [ ] 實作中文條件抽取。
- [ ] 實作 Metadata Filter。
- [ ] 實作語意 Top-N 檢索。
- [ ] 將候選送入庫存比對。
- [ ] 產生有根據的推薦說明。
- [ ] 在說明中保留 `model_id`、完成率與缺件證據。
- [ ] 建立以 `object_id` 隔離的 Train-only 檢索索引。
- [ ] 實作 F-pipeline 的 Top-N 形狀候選與 CP-SAT 接口。
...
- 核心版能從手動庫存與文字走到推薦、E 生成或 F-pipeline 結果。
- 所有錯誤有可理解的提示，不直接顯示內部 Traceback。
...
- [ ] Train-only Retrieval Leakage。（RAG 階段才需要）
- [x] CP-SAT 決定性。（**多執行緒下同 seed 不可重現**，故預設 `workers=1`；
      實測單執行緒反而更快：中位數 0.108s vs 0.128s）
- [x] LDraw 座標與旋轉輸出。（黃金向量對齊官方）
- [ ] 推薦分數與缺件。（引擎層 `missing()` 已測，推薦排序未做）

目前狀態：公開化審查工作樹 **550 passed、30 skipped**（離線同結果）。
...
- [ ] 生成到硬性驗證。
- [ ] 生成到 LDraw。
- [ ] Voxel 到 CP-SAT re-tiling 再到結構 Checker。
- [ ] Test Query 到 Train-only Retrieval 再到 CP-SAT 的 F-pipeline。
- [ ] 無色結構到顏色庫存指派。
- [ ] 圖片到可修正庫存。
- [ ] 圖片庫存到推薦。
- [ ] 圖片庫存到生成。
...
- [ ] 手動輸入庫存。
- [ ] 既有作品比對。
- [ ] BrickGPT 公開模型完成文字生成、解析與 `.ldr`。
- [ ] 8 種零件旋轉正規化與庫存引擎。
- [ ] CP-SAT re-tiling 反事實資料。
- [ ] 自行完成一次 LoRA／QLoRA 微調。
- [ ] A～E 比較，包含 D／E 主要 Ablation。
- [ ] F-oracle 與 F-pipeline Baseline。（**F-oracle 已完成**；F-pipeline 未開始）
- [ ] 硬性庫存約束解碼與明確終止策略。
- [ ] 碰撞與基本連接檢查。
- [ ] 3D 圖片或預覽。
- [ ] LDraw 輸出。
...
> 最小兩頁式 UI 能從手動庫存與文字得到推薦、E 或 F-pipeline 結果、驗證資訊、顏色指派、3D 與 LDraw。
...
- [ ] 建立正式程式 Repository 結構。
- [ ] 執行 BrickGPT 最小推論與 `.ldr` 煙霧測試。
- [ ] 下載並分析 StableText2Brick。
- [ ] 實作庫存 Schema 與引擎。
- [ ] 實作既有作品推薦。
- [ ] 執行 A／B 簡化生成 Baseline。
- [ ] 建立 CP-SAT re-tiling 與 F 系列 Baseline。
- [ ] 建立硬性約束引擎。
- [ ] 建立庫存條件微調資料。
- [ ] 完成 LoRA／QLoRA。
- [ ] 核心完成後再訓練 8 類圖片分類與物件偵測。
- [ ] 實作 RAG、UI、LDraw 與組裝步驟。
- [ ] 核心完成後視情況執行 BrickNet 0.6B 公開模型。
